...
<script>
import { postgoodsaddreq, getcategoriesreq } from "@/api/axios";
export default {
  name: "xtkj",
  data() {
    ...
    return {
      ...
      rules: {
        ...
        region: [{ validator: validateclassfy, trigger: "blur" }],
        // type: [{ validator: validatePass, trigger: "blur" }],
        // resource: [{ validator: validatePass, trigger: "blur" }],
        desc: [{ validator: validatetips, trigger: "blur" }],
      },
    };
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          let res = await postgoodsaddreq({
            name: this.ruleForm.name,
            category: this.ruleForm.region,
            price: this.ruleForm.price,
            imgUrl: this.ruleForm.imgname,
    ...
    picname(response, file) {
      this.ruleForm.imgname = file.response.imgUrl;
    },
  },
  created() {
